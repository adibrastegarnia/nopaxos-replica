...
func TestRaftProtocol(t *testing.T) {
	...
	roles := make(map[RoleType]func(NOPaxos) Role)
	nopaxos := newNOPaxos(NewCluster(cluster), &config.ProtocolConfig{ElectionTimeout: &electionTimeout}, &unimplementedClient{}, roles, store)
	assert.Equal(t, StatusStopped, nopaxos.Status())
	statusCh := make(chan Status, 1)
	nopaxos.Watch(func(event Event) {
	...
	nopaxos.WriteLock()
	nopaxos.Init()
	assert.Equal(t, StatusRunning, nopaxos.Status())
	assert.Equal(t, StatusRunning, <-statusCh)
	nopaxos.WriteUnlock()

	// Verify the cluster configuration
	assert.Equal(t, electionTimeout, nopaxos.Config().GetElectionTimeoutOrDefault())
	assert.Equal(t, MemberID("foo"), nopaxos.Member())
	assert.Len(t, nopaxos.Members(), 3)
	assert.Equal(t, MemberID("foo"), nopaxos.GetMember(MemberID("foo")).MemberID)
	assert.Equal(t, MemberID("bar"), nopaxos.GetMember(MemberID("bar")).MemberID)
	assert.Equal(t, MemberID("baz"), nopaxos.GetMember(MemberID("baz")).MemberID)

	// Verify the initial values
	assert.Equal(t, foo, nopaxos.Member())
	assert.Len(t, nopaxos.Members(), 3)
	assert.Equal(t, Term(0), nopaxos.Term())
	assert.Nil(t, nopaxos.Leader())
	assert.Nil(t, nopaxos.LastVotedFor())
	assert.Equal(t, Index(0), nopaxos.CommitIndex())

	// Verify that the term cannot be decreased
	assert.NoError(t, nopaxos.SetTerm(Term(1)))
	assert.Equal(t, Term(1), nopaxos.Term())
	assert.NoError(t, nopaxos.SetTerm(Term(3)))
	assert.Equal(t, Term(3), nopaxos.Term())
	assert.Error(t, nopaxos.SetTerm(Term(2)))

	// Verify that only one vote can be cast per term
	assert.NoError(t, nopaxos.SetLastVotedFor(foo))
	assert.Equal(t, &foo, nopaxos.LastVotedFor())
	assert.Error(t, nopaxos.SetLastVotedFor(bar))
	assert.Equal(t, &foo, nopaxos.LastVotedFor())
	assert.NoError(t, nopaxos.SetTerm(Term(4)))
	assert.Nil(t, nopaxos.Leader())
	assert.Nil(t, nopaxos.LastVotedFor())
	assert.Error(t, nopaxos.SetLastVotedFor("none"))
	assert.Nil(t, nopaxos.LastVotedFor())
	assert.NoError(t, nopaxos.SetLastVotedFor(bar))
	assert.Equal(t, &bar, nopaxos.LastVotedFor())
	assert.Error(t, nopaxos.SetLastVotedFor(""))
	assert.Equal(t, &bar, nopaxos.LastVotedFor())

	// Verify that the leader cannot be changed
	assert.Nil(t, nopaxos.Leader())
	assert.NoError(t, nopaxos.SetLeader(&bar))
	...
	roles = map[RoleType]func(NOPaxos) Role{
		RoleFollower: func(r NOPaxos) Role {
			return follower
		},
		RoleLeader: func(r NOPaxos) Role {
			return leader
		},
	}
	raft = newNOPaxos(NewCluster(cluster), &config.ProtocolConfig{}, &unimplementedClient{}, roles, store)
	assert.Equal(t, StatusStopped, raft.Status())
	raft.WriteLock()
	raft.Init()
	assert.Equal(t, StatusRunning, raft.Status())
