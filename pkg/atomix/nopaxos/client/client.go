...
func NewClient(config cluster.Cluster, consistency nopaxos.ReadConsistency) *Client {
	cluster := nopaxos.NewCluster(config)
	return newClient(cluster, nopaxos.NewClient(cluster), consistency)
}

// newClient returns a new NOPaxos client
func newClient(cluster nopaxos.Cluster, client nopaxos.Client, consistency nopaxos.ReadConsistency) *Client {
	members := list.New()
...
type Client struct {
	...
	leader      *nopaxos.MemberID
	client      nopaxos.Client
	consistency nopaxos.ReadConsistency
	mu          sync.RWMutex
	log         util.Logger
}
...
func (c *Client) getLeader() nopaxos.MemberID {
	c.mu.RLock()
	if c.leader != nil {
		defer c.mu.RUnlock()
		return *c.leader
...
func (c *Client) resetLeader(expected nopaxos.MemberID, leader *nopaxos.MemberID) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) write(ctx context.Context, request *nopaxos.CommandRequest, ch chan<- node.Output) error {
	go c.sendWrite(ctx, request, ch)
	return nil
}

// retryWrite retries a write request
func (c *Client) retryWrite(ctx context.Context, request *nopaxos.CommandRequest, ch chan<- node.Output, leader nopaxos.MemberID) {
	c.resetLeader(leader, nil)
	go c.sendWrite(ctx, request, ch)
}

// sendWrite sends a write request
func (c *Client) sendWrite(ctx context.Context, request *nopaxos.CommandRequest, ch chan<- node.Output) {
	leader := c.getLeader()
...
func (c *Client) receiveWrite(ctx context.Context, request *nopaxos.CommandRequest, ch chan<- node.Output, leader nopaxos.MemberID, stream <-chan *nopaxos.CommandStreamResponse) {
	for streamResponse := range stream {
		...
		if response.Status == nopaxos.ResponseStatus_OK {
			ch <- node.Output{
				Value: response.Output,
			}
		} else if response.Error == nopaxos.ResponseError_ILLEGAL_MEMBER_STATE {
...
func (c *Client) getMember() nopaxos.MemberID {
	c.mu.RLock()
	if c.member == nil {
		c.mu.RUnlock()
		return c.getMemberSafe()
...
func (c *Client) getMemberSafe() nopaxos.MemberID {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.member == nil {
		...
		member := c.memberNode.Value.(nopaxos.MemberID)
		c.member = &member
	}
	return *c.member
}
...
func (c *Client) read(ctx context.Context, request *nopaxos.QueryRequest, ch chan<- node.Output) error {
	go c.sendRead(ctx, request, ch)
	return nil
}

// retryRead retries a read request
func (c *Client) retryRead(ctx context.Context, request *nopaxos.QueryRequest, ch chan<- node.Output) {
	c.resetMember()
	go c.sendRead(ctx, request, ch)
}

// sendRead sends a read request
func (c *Client) sendRead(ctx context.Context, request *nopaxos.QueryRequest, ch chan<- node.Output) {
	member := c.getMember()
...
func (c *Client) receiveRead(ctx context.Context, request *nopaxos.QueryRequest, ch chan<- node.Output, member nopaxos.MemberID, stream <-chan *nopaxos.QueryStreamResponse) {
	for streamResponse := range stream {
		if streamResponse.Failed() {
